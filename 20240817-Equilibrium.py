...
def EquilibriumFunc(Input_V,Pot_V,Directory_V,Basis_V):
    ...
    with fileinput.FileInput(LammpsInFileAddress, inplace=True) as file:
        for line in file:
            ...
            line = line.replace("PotentialTemp", str(Pot_V))
    ...
    if SimulationEnvironment=="Windows":
        ...
        elif Pot_V in ["TabGap1", "TabGap2"]:
            srun_command = [
                "srun",
                "/home/veshand/Software/Lammps-tabGAP/lmp4_CC",
                "-in", LammpsInFileAddress,
                "-log", LammpsLogFileAddress,
                # ">", "/", "dev", "/" "null", "2", ">", "&", "1"
            ]
        #print(srun_command)
        subprocess.run(srun_command, check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
...
if Active:
    for Pot in PotList:
        # <editor-fold desc="Run">
        IterationDf = pd.DataFrame(
            columns=['Num', 'A1X', 'A2Y', 'A3Z', 'PressureX', 'PressureY', 'PressureZ', 'PressureSum', 'Potential'])
        print(Pot)
        ...
        Num = 0
        try:
            PressureMinimize = minimize(
                EquilibriumFunc, BoxInitial, args=Arguments, method="Nelder-Mead",
                options={"disp": True, "maxiter": MinimizeMaxiter},
                tol=MinimizeTol,
            )
        except ConvergedEarly as e:
            print("Stopped early:", e)
            PressureMinimize = None  # or keep the last best Input_V
